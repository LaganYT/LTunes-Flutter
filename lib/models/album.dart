...
class Album {
  ...
  factory Album.fromJson(Map<String, dynamic> json) {
    ...
    final albumId =
        albumData['ALB_ID']?.toString() ?? json['id']?.toString() ?? '';
    final albumTitle = albumData['ALB_TITLE']?.toString() ??
        json['title']?.toString() ??
        'Unknown Album';
    final albumArtPicId = albumData['ALB_PICTURE']?.toString() ??
        json['albumArtPictureId']?.toString() ??
        '';
    ...
    String artistName = albumData['ART_NAME']?.toString() ??
    ...
    if (artistName.isEmpty) {
      ...
      if (artistsList != null && artistsList.isNotEmpty) {
        ...
        artistName =
            firstArtistMap?['ART_NAME']?.toString() ?? 'Unknown Artist';
      } else {
    ...
    List<Song> songs;
    final rawTracksField = json[
        'tracks']; // This could be List<map> (our save), Map (Deezer API {'data': [...]}), or null
    ...
    if (json['isSaved'] as bool? ?? false) {
      ...
      final List<dynamic> tracksList =
          (rawTracksField is List) ? rawTracksField : [];
      songs = tracksList
      ...
      if (rawTracksField is List &&
          rawTracksField.isNotEmpty &&
      ...
      } else if (rawTracksField is Map &&
          rawTracksField.containsKey('data') &&
          rawTracksField['data'] is List) {
      ...
      if (useSongFromJsonForApiOrOldFormat) {
        songs = trackListForParsing
            .map(
                (trackJson) => Song.fromJson(trackJson as Map<String, dynamic>))
            .toList();
      } else {
        // Use Song.fromAlbumTrackJson for API-specific track formats.
        songs = trackListForParsing.map((trackJson) {
          return Song.fromAlbumTrackJson(trackJson as Map<String, dynamic>,
              albumTitle, albumArtPicId, albumReleaseDate, artistName, albumId);
        }).toList();
      }
    }
    ...
    return Album(
      ...
      durationSeconds: int.tryParse(albumData['DURATION']?.toString() ??
          json['durationSeconds']?.toString() ??
          ''),
      trackCount: int.tryParse(albumData['NUMBER_TRACK']?.toString() ??
          json['trackCount']?.toString() ??
          ''),
      isSaved: json['isSaved'] as bool? ?? false, // Load isSaved
      playCount: json['playCount'] as int? ?? 0, // Load playCount
      localAlbumArtUrl:
          json['localAlbumArtUrl'] as String?, // Load localAlbumArtUrl
    );
  }
