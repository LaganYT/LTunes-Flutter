...
class AudioPlayerHandler extends BaseAudioHandler
  ...
  void _configureAudioPlayer() {
    // Basic audio player configuration
    // AudioService handles most optimizations automatically
  }
    ...
    _audioPlayer.positionStream.listen((position) async {
      ...
      if (position > Duration.zero &&
        ...
        if (currentItem != null && currentItem.duration != null) {
          ...
          if (timeRemaining.inMilliseconds <= 300) {
  ...
  Future<void> skipToNext() async {
    ...
    debugPrint("AudioHandler: skipToNext from $_currentIndex to $newIndex");
  ...
  Future<void> skipToPrevious() async {
    ...
    if (newIndex < 0) {
      if (playbackState.value.repeatMode == AudioServiceRepeatMode.all) {
        ...
        newIndex = _playlist.length - 1; // Loop to last song
      }
    }

    debugPrint("AudioHandler: skipToPrevious from $_currentIndex to $newIndex");
    await skipToQueueItem(newIndex);
  }
  ...
  Future<void> _handleSongCompletion() async {
    ...
    debugPrint("AudioHandler: Song completion - Repeat: $repeatMode, Index: $_currentIndex");

    if (repeatMode == AudioServiceRepeatMode.one) {
      if (_currentIndex >= 0 && _currentIndex < _playlist.length) {
        debugPrint("AudioHandler: Repeating single song");
        await _prepareToPlay(_currentIndex);
      ...
      if (_currentIndex == _playlist.length - 1) {
        if (repeatMode == AudioServiceRepeatMode.all) {
          ...
          debugPrint("AudioHandler: Looping queue - moving from last song to first");
          ...
          debugPrint("AudioHandler: End of queue reached");
          await _audioPlayer.stop();
          playbackState.add(playbackState.value.copyWith(
            playing: false,
        ...
        // Move to next song
        debugPrint("AudioHandler: Moving to next song");
        await skipToNext();
      }
    }
  }
